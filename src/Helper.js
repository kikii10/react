...
/*hago una copia para meter el ultimo movimiento asi no rompo todo el array(lo hago asi sino solo funcionaba aveces)*/
    let board = [...gameBoard];
...
/*revisa si hay alguna de las convinaciones ganadoras de arriba y retorna un booleano*/ 
...
/*esto hace que cuando no queden mas espacios en el array devuelve un booleano mostrando que se dio un empate*/ 
export const empate = (gameBoard,currentMove,currentPlayer) =>{
    let board = [...gameBoard];
    board[currentMove] = currentPlayer;
   ...
   /*crea un numero aleatorio entre los que quedan libres en el array*/
...
/*este es el buscador de la mejor jugada usando los movechecks de la funcion de abajo */
...
for (let check = 0; check < moveChecks.length; check++){
    ...
    for (let i = 0 ; i<moveChecks[check].max; i += moveChecks[check].step){
        let series =
               ...
               /*aca se busca entre todos los casos donde un circulo significa gganar o perder asi retorna donde ponerlo */
                switch (series) {
                    case "1110":
                    case "2220":
...
export const computerMove = (gameBoard) => {
    ...
    let moveChecks = [
        /*pruevo los verticales*/
        {
            indexes: [0, 4, 8, 12],
            max: 4,
            step: 1,
        },
        /*pruevo los horizontales*/
        {
            indexes: [0, 1, 2, 3],
            max: 16,
            step: 4,
